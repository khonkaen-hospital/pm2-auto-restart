...
function reload_all(){
	...
	if (lastPm2Name != row['name']){
		...
		if ((fs.existsSync(fileName))){
		    ...
		    // console.log(row['name'],':', chalk.red.bold('Not found auto load file.'));
		}
    	    }
	}
    }
...
function restart_pm2(pm2Name) {
    ...
    let processList = getPM2Process(pm2Name);
//    var processList = getPM2Process(pm2Name);
    for (let row of processList){
	console.log('  -> restarting process '+row['pm_id']);
        var jlist = shell.exec('pm2 restart '+row['pm_id']+' >null', { silent: true });
    }
    listPM2(pm2Name)
}
...
function listPM2(pm2Name){
	...
	cpu += +row['monit']['cpu'];
    	console.log(
    	    row['pm2_env']['status'], row['pid'], row['name'], row['pm_id']
    	    , new Date(row['pm2_env']['pm_uptime']).toString()
    	    , 'MEM:'+(row['monit']['memory']/1024/1024).toFixed(2)+'MB'
    	    , 'CPU:'+(row['monit']['cpu'])+'%'
    	);
    }
    console.log('='.repeat(100));
    console.log('Total '+
    	    'MEM:'+(mem/1024/1024).toFixed(2)+'MB'
    	    , 'CPU:'+cpu.toFixed(2)+'%'
    	);
}
...
function getPM2Process(pm2Name) {    
    ...
    var jlist = shell.exec('sudo pm2 jlist', { silent: true });
